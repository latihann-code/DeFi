...
from defi_agent.brain.math_models import calculate_expected_value, calculate_kelly_size
...
class BacktestEngine:
    ...
    def execute_trade(self, timestamp: int, pool: PoolData, action: str, amount: float) -> bool:
        ...
        self.trade_logs.append({"ts": timestamp, "pool": pool.pool, "act": action, "amt": amount, "total": self.get_total_equity()})
        return True
    ...
    def run_simulation(self, multi_pool_data: Dict[str, List[Dict]], baseline_apy: float = 5.0):
        ...
        for i, ts in enumerate(all_ts):
            ...
            for p_id, hourly_list in multi_pool_data.items():
                ...
                if data:
                    current_map[p_id] = PoolData(
                        ...
                        tvl_drop_24h_percent=data.get("tvl_drop_24h", 0.0), age_days=100
                    )
            ...
            for p_id, pool in current_map.items():
                ...
                if ev > 0:
                    # Kelly sizing: divide total portfolio
                    target_pct = calculate_kelly_size(ev, total_equity * 0.1)
                    target_amt = total_equity * target_pct
                    current_amt = self.positions[p_id].amount if p_id in self.positions else 0
                    
                    if target_amt > (current_amt + 500):
                        self.execute_trade(ts, pool, "DEPOSIT", target_amt - current_amt)
