...
def test_engine_cooldown():
    ...
    success1 = engine.execute_trade(timestamp=0, pool=pool, action="DEPOSIT", amount=5000)
    assert success1 is True
    
    # Trade kedua di pool yang sama (Jam ke-1) -> Harus GAGAL karena cooldown
    success2 = engine.execute_trade(timestamp=3600, pool=pool, action="DEPOSIT", amount=5000)
    assert success2 is False
    
    # Trade ketiga (setelah 24 jam) -> Harus BERHASIL
    success3 = engine.execute_trade(timestamp=3600*25, pool=pool, action="DEPOSIT", amount=5000)
...
def test_stage_1_loop():
    ...
    # Jalankan engine
    engine.run_stage_1(historical_data, pool_id="1", project="aave")
